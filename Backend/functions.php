...
function team_count(){
    ...
    $query = "SELECT name FROM team WHERE member!='former'";
    $result = mysqli_query($connection, $query);
    confirmQuery($result);
...
function sub_count(){
    ...
    $query = "SELECT subject FROM subject WHERE status!='review'";
    $result = mysqli_query($connection, $query);
    confirmQuery($result);
    return mysqli_num_rows($result) ;
}
...
function register_team($name, $email, $role,$batch,$pos){
    ...
    global $connection;
        ...
        $query = "INSERT INTO team (name, email, role,position,batch) ";
        $query .= "VALUES('{$name}','{$email}', '{$role}', '{$pos}', '{$batch}')";
